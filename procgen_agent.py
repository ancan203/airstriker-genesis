...
class DQNAgent:
    ...
    def load(self, load_path, reset_exploration_rate, load_replay_buffer):
        ...
        if load_replay_buffer:
            ...
            print(
                f"Loading replay memory. Len {len(replay_memory)}" if replay_memory else "Saved replay memory not found. Not restoring replay memory.")
            self.memory = replay_memory if replay_memory else self.memory

        if reset_exploration_rate:
            print(
                f"Reset exploration rate option specified. Not restoring saved exploration rate {exploration_rate}. The current exploration rate is {self.exploration_rate}")
        else:
